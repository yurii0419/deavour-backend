...
describe('Campaign Shipping Destination actions', () => {
  ...
  describe('Delete a campaign shipping destination by id', () => {
    it('Should return 204 No Content when an admin successfully deletes a shipping destination.', async () => {
      const resCompany = await chai
        ...
        .send({
          company: {
            name: 'Captain Marvel Company Campaign Shipping Address',
            email: 'ivers@kree.kr'
          }
        })
      ...
      const resShippingDestination = await chai
        .request(app)
    ...
    it('Should return 403 Forbidden when a campaign manager tries to delete a shipping destination.', async () => {
    ...
    it('Should return 403 Forbidden when a company admin who is not an employee tries to delete a shipping destination.', async () => {
      const resCompany = await chai
        .request(app)
        .post('/api/companies')
        .set('Authorization', `Bearer ${tokenAdmin}`)
    ...
    it('Should return 403 Forbidden when a user who is not an employee tries to delete a shipping destination.', async () => {
      const resCompany = await chai
        .request(app)
        .post('/api/companies')
        .set('Authorization', `Bearer ${tokenAdmin}`)
    ...
    it('Should return 403 Forbidden when a company non owner tries to delete a shipping destination', async () => {
      const resCompany = await chai
        .request(app)
        .post('/api/companies')
        .set('Authorization', `Bearer ${tokenAdmin}`)
