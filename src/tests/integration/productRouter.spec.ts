...
describe('Product actions', () => {
  ...
  describe('Product Graduated Prices of Children', () => {
    it('Should return 200 OK when an admin adds a graduated price to a children product', async () => {
      const resProductParent = await chai
        .request(app)
        .post('/api/products')
        .set('Authorization', `Bearer ${tokenAdmin}`)
    ...
    it('Should return 404 Not Found when an admin tries to add a empty graduated price to a children product', async () => {
      ...
      expect(res).to.have.status(404)
      expect(res.body).to.include.keys('statusCode', 'success', 'errors')
      expect(res.body.errors.message).to.equal('No graduated Prices in Parent Product')
    })

    it('Should return 404 Not Found when an admin tries to add the graduated prices to a empty children product', async () => {
      const resProductParent = await chai
        .request(app)
        .post('/api/products')
        .set('Authorization', `Bearer ${tokenAdmin}`)
      ...
      expect(res).to.have.status(404)
      expect(res.body).to.include.keys('statusCode', 'success', 'errors')
      expect(res.body.errors.message).to.equal('Child product not found')
    })
  })
  ...
  describe('Get product customisations by id', () => {
    it('Should return 200 OK when an admin successfully retrieves all product customisations with productId.', async () => {
      ...
      const res = await chai
        .request(app)
        .get(`/api/products/${productId}/product-customisations/${String(productCustomiationRes.body.productCustomisation.id)}`)
        .set('Authorization', `Bearer ${tokenAdmin}`)
  ...
  describe('Delete product customisation', () => {
    it('Should return 204 when a admin deletes a product customisation.', async () => {
      ...
      const res = await chai
        .request(app)
        .delete(`/api/products/${productId}/product-customisations/${String(resProductCustomisation.body.productCustomisation.id)}`)
        .set('Authorization', `Bearer ${tokenAdmin}`)
    ...
    it('Should return 204 when a user deletes a product customisation.', async () => {
      ...
      const res = await chai
        .request(app)
        .delete(`/api/products/${productId}/product-customisations/${String(productCustomisationId)}`)
        .set('Authorization', `Bearer ${token}`)
    ...
    it('Should return 404 when a admin tries to delete a product customisation that does not exist.', async () => {
      const res = await chai
        .request(app)
        .delete(`/api/products/${productId}/product-customisations/88D48647-ED1C-49CF-9D53-403D7DAD8DB7`)
        .set('Authorization', `Bearer ${tokenAdmin}`)
  ...
  describe('Update a product customisation', () => {
    it('Should return 200 OK when an user updates a product customisation by id.', async () => {
      ...
      const res = await chai
        .request(app)
        .put(`/api/products/${productId}/product-customisations/${String(productCustomisationId)}`)
        .set('Authorization', `Bearer ${token}`)
        .send({
          productCustomisation: {
            customisationType: 'print',
  ...
  describe('Create and Get a product customiation chat', () => {
    it('Should return 201 CREATED when an user create chat', async () => {
      ...
      const res = await chai
        .request(app)
        .post(`/api/products/${productId}/product-customisations/${String(productCustomisationId)}/chat`)
        .set('Authorization', `Bearer ${token}`)
        .send({
    ...
    it('Should return 200 OK when an user get all chat', async () => {
      ...
      const res = await chai
        .request(app)
        .get(`/api/products/${productId}/product-customisations/${String(productCustomisationId)}/chat`)
        .set('Authorization', `Bearer ${token}`)
